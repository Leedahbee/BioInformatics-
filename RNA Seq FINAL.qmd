...
Use model.matrix to tell edgeR the experimental design and the variables we are testing for. Next estimateDisp is used to estimate the spread of the count data. Applying glmQLFit so our data fits a generalized linear model. The glmQLFTest line performs a likelihood ratio test to identify genes that are differentially expressed. This helps to identify which genes are significantly different from others in the sample. Finally topTags displays the differentially expressed genes.
